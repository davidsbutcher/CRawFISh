#' Save raw file info
#'
#' @name save_raw_file_info
#' @param rawFileInfo Dataframe containing all raw file summary data.
...
save_raw_file_info <-
   function(
      ...
      writeData(xl_workbook, 1, rawFileInfo, withFilter = TRUE)
      freezePane(xl_workbook, 1, firstRow = TRUE)
      setColWidths(xl_workbook, 1, cols = 1:length(rawFileInfo), widths = "auto")
      ...
      addStyle(
         ...
         rows = 1, cols = 1:length(rawFileInfo)
      )
      ...
      if (use_tdreport == TRUE) {
         ...
         saveWorkbook(
            xl_workbook,
            file = paste0(outputDir, "/", path_ext_remove(tdReportName), "_CRawFISh_raw_info.xlsx"),
            overwrite = TRUE
         )
         ...
         saveWorkbook(
            xl_workbook,
            file = paste0(outputDir, "/", systime, "_CRawFISh_raw_info.xlsx"),
            overwrite = TRUE
         )
